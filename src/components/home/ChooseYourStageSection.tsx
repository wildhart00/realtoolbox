  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {STAGES.map((s) => {
          const Button = (
            <span className="inline-flex items-center justify-center rounded-[10px] bg-gradient-to-r from-[hsl(239_84%_60%)] via-[hsl(252_84%_64%)] to-[hsl(265_84%_60%)] px-4 py-2.5 text-[13px] font-semibold text-white shadow-lg shadow-[hsl(252_84%_50%)]/20 hover:shadow-[hsl(252_84%_50%)]/35 transition-base">
              {s.buttonLabel} →
            </span>
          );
          return (
            ...
            >
              ...
              <div className="mt-auto">
                {s.to ? (
                  <Link to={s.to}>{Button}</Link>
                ) : (
                  <a href={s.href}>{Button}</a>
                )}
              </div>
            </div>
          );
