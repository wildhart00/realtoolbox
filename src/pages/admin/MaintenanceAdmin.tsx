...
export default function MaintenanceAdmin() {
  ...
  const failed = useMemo(() => result?.results.filter((r) => !r.ok) ?? [], [result]);
    ...
    try {
      while (true) {
        ...
        setProgress({
          ...
          total: onlyMissing
            ? aggregated.processed + (batch.hasMore ? batch.total : 0)
            : batch.total,
        });
        if (!batch.hasMore || batch.processed === 0) break;
        offset = batch.nextOffset;
      }
      toast.success(`Refreshed ${aggregated.succeeded}/${aggregated.processed} tool images`);
    } catch (e) {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <section className="rounded-xl border border-border/60 bg-card p-6">
        ...
        <Button onClick={runRefresh} disabled={running} className="mt-4 gap-2">
          {running ? (
